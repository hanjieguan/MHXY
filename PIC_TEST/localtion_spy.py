...
def test():
    while True:
        ...
        # im = Image.open('E:\mhxypic\mlocaltion/1623531759.png')
        ...
        try:
            try:
                ...
                while True:
                    ...
                    local_name_thitck = Image_array[0:14, local_start_column - 1:local_start_column]
                    for local_name_thitck_element in local_name_thitck:
                        if np.array_equal(local_name_thitck_element, np.array([[255, 255, 255]])) or np.array_equal(local_name_thitck_element, np.array([[0, 0, 0]])):
                            local_or_not = True
                            break
                    ...
                    if local_or_not is True:
                        ...
                        if local_name_nums == 1:
                            ...
                            if font_test_list3 in localtion_spy_font.Four_font_list:
                                if (first_font + second_font + three_font + font_test_list3) in localtion_spy_font.Four_word_list:
                        ...
                        if local_name_nums == 0:
                            ...
                            localtion_now = localtion_spy_font.All_place_wenzi_list[localtion_spy_font.All_place_list.index(first_font+second_font+three_font)]
                            # 检测X坐标
                            for local_pos in range(18, 42, 8):
                                if len(local_x_list) == 0:
                                    local_pos_font = Image_array[51:61, local_pos:local_pos + 8]
                            ...
                            localtion_now = localtion_spy_font.All_place_wenzi_list[localtion_spy_font.All_place_list.index(first_font + second_font + three_font + four_font)]
                            # 检测X坐标
                            for local_pos in range(18, 42, 8):
                                if len(local_x_list) == 0:
                                    local_pos_font = Image_array[51:61, local_pos:local_pos + 8]
...
def test2():
    for root, dirs, files in os.walk('E:\mhxypic\mlocal_single/'):
        for file in files:
            im = Image.open('E:\mhxypic\mlocal_single/'+ file)
            Image_array = np.array(im)
            font_test_list3 = []
...
def test3():
    im = Image.open('E:\mhxypic\mlocal_single/Y.png')
    Image_array = np.array(im)
    font_test_list3 = []
    for font_array_2d in Image_array:
        font_test_list2 = []
