                                ...
                                while orginal_column < 138:
                                    ...
                                    for font_single_columns in range(0, 3):
                                        if single_font_signal == 1:
                                            ...
                                            now_font_tictlk = Image_array[font_rows:font_rows + 14,orginal_column + font_single_columns:orginal_column + font_single_columns + 1]
                                            for now_font_tictlk_element in now_font_tictlk:
                                                if single_font_signal == 1:
                                                    break
                                                else:
                                    ...
                                    if redword_time == 2:
                                        ...
                                        if single_font_none_signal != 0:
                                            if now_font_color == 0:
                                                ...
                                                # 此时的红字为数字，即坐标，第三次才出现坐标，且未普通任务，挨个获取坐标信息
                                                # 获取当前数字信息
                                                font_test = Image_array[font_rows:font_rows + 14, orginal_column:orginal_column + 6]
                                                font_test_list3 = []
                                                for font_test_2d in font_test:
                                                    font_test_list2 = []
                                                    for font_test_1d in font_test_2d:
...
def test():
    ...
    # im = ImageGrab.grab((489, 174, 629, 259))
    im = Image.open('E:\mhxypic\mtask3\m2.png')
    Image_array = np.array(im)
    # 挖字位置 以下都用
    font_test = Image_array[0:7, 0:13]
    font_test_list3 = []
                    ...
                    for font_columns in range(14, 127, 14):
                        ...
                        for font_single_columns in range(0, 3):
                            if single_font_signal == 1:
                                ...
                                now_font_tictlk = Image_array[15:29, font_columns+font_single_columns:font_columns+font_single_columns+1]
                                for now_font_tictlk_element in now_font_tictlk:
                                    if single_font_signal == 1:
                                        break
                                    else:
                            ...
                            if now_font_color == 1:
                                ...
                                if redword_time == 1:
                                    ...
                                    if font_test_list3 == task_spy_font.RED_qiang:
                                        ...
                                        # 此时找出 挖宝场景
                                        wb_place = task_spy_font.Local_wenzi_list[task_spy_font.Local_list.index(font_test_list3)]
                                    else:
                                        raise KeyboardInterrupt
                                # 第一行红字最多出现两次，所以else为第二次，因为肯定大于0,且不可能为数字
                                else:
                                        ...
                                        # 当前为红字
                                            ...
                                            # 此时的红字为数字，即坐标
                                                ...
                                                for font_test_2d in font_test:
                                                    ...
                                                    font_test_list3.append(font_test_list2)
                                                if font_test_list3 in task_spy_font.Num_list:
                                                    font_test = task_spy_font.Num_num_list[task_spy_font.Num_list.index(font_test_list3)]
                                                    if red_comma == 0:
                                                        coor_x_list.append(font_test)
                                                    else:
                                                        coor_y_list.append(font_test)
...
def test2():
    im = Image.open('E:\mhxypic\mtask4/8.png')
    font_wa = np.array(im)
    font_test = font_wa[0:14, 0:6]
    # print(font_test)
    font_test_list3 = []
    for font_test_2d in font_test:
        font_test_list2 = []
